...
vmin = 1.33      # color bar min
vmax = 1.53      # color bar max
...
def plotBonds(bb):
    for (q1, q2) in bb:
        ...
        if (showLengths == True):
            plt.text((xs[q1] + dx / 2) - 0.4, (ys[q1] + dy / 2) + 0.1, str(rnd), fontsize=15, fontweight='bold',
                     bbox=props)
        # plt.colorbar(im)
        # plt.text()
...
fil0 = genfromtxt(sys.argv[1], skip_header=2)
data,target = np.array_split(np.loadtxt(sys.argv[1], skiprows=2, dtype=str), [-1], axis=1)
xs = fil0[:, 1]
ys = fil0[:, 2]
zs = fil0[:, 3]
...
fname2 = os.path.splitext(sys.argv[1])[0]
fig = plt.figure(figsize=(2*deltaX / 1,1.5* deltaY /1 ))
plt.subplot(1, 1, 1)
...
plt.savefig(fname2 + "_.png", format='png', dpi=200)  # ,bbox_inches='tight'
plt.show()
plt.close()
print("ALL DONE!")
